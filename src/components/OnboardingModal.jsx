...
import { Check, ChevronRight, Phone, Calendar, Loader2 } from 'lucide-react';
import { useCreateBusinessMutation } from '../slices/apiSlice/businessApiSlice';
import { useCreateAssistantMutation } from '../slices/apiSlice/assistantApiSlice';
...
import { useGetOnboardingStatusQuery, useCompleteOnboardingMutation } from '../slices/apiSlice/onboardingApiSlice';
...
export default function OnboardingModal({ onComplete }) {
  ...
  // Handle OAuth callback - check for google=success in URL params
  useEffect(() => {
    ...
    if (googleParam === 'success') {
      ...
      // Clear the param and refetch status
  ...
  useEffect(() => {
    if (onboardingStatus) {
      // Use nextStep from backend - this is the step user should work on
  ...
  const handleSaveBusiness = async () => {
    ...
    try {
      ...
      // Refetch status to get updated state and move to next step
      const result = await refetchStatus();
      if (result.data) {
        setCurrentStep(result.data.nextStep || 2);
      }
    } catch (err) {
      toast.error(err?.data?.error?.message || err?.data?.message || 'Failed to create business');
    }
  };
  ...
  const handleCreateAssistant = async () => {
    ...
    try {
      ...
      // Refetch status to get updated state
      const result = await refetchStatus();
      if (result.data) {
        setCurrentStep(result.data.nextStep || 3);
      }
    } catch (err) {
      toast.error(err?.data?.error?.message || err?.data?.message || 'Failed to create assistant');
    }
  };

  // Step 3: Phone (order changed - phone before calendar)
  const handleSavePhone = async () => {
    const businessId = onboardingStatus?.businessId;
    const assistantId = onboardingStatus?.assistantId;
    ...
    if (phoneTab === 'new') {
      ...
      try {
        ...
        const result = await refetchStatus();
        if (result.data) {
          setCurrentStep(result.data.nextStep || 4);
        }
      } catch (err) {
      ...
      // For existing phone, just move to next step
      setCurrentStep(4);
    }
  };
  ...
  const handleConnectCalendar = async () => {
    ...
    try {
      // Pass source=onboarding so callback redirects back to onboarding
      const result = await connectGoogleCalendar({ businessId, source: 'onboarding' }).unwrap();
  ...
  const handleSkipCalendar = () => {
    setCurrentStep(4); // Go to phone step
  };
  ...
  const handleTestCall = async () => {
    ...
    try {
      await testCall({ businessId, phoneNumber: testPhoneNumber.trim() }).unwrap();
  ...
  const isLoading = isCreatingBusiness || isCreatingAssistant || isPurchasingNumber || isConnectingCalendar || isCompletingOnboarding;
  ...
  // Extract status values for display
  const { hasBusiness, hasAssistant, hasCalendarConnected, hasPhoneNumber } = onboardingStatus || {};
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#1a1a1d] rounded-lg border border-[#303030] w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4">
              ...
              {hasBusiness ? (
                <div className="bg-[#111114] border border-[#303030] rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white font-medium">Business already created</p>
          ...
          {currentStep === 2 && (
            <div className="space-y-5">
              ...
              {hasAssistant ? (
                <div className="bg-[#111114] border border-[#303030] rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    <Check className="w-5 h-5 text-green-500" />
                    <div>
                      <p className="text-white font-medium">Assistant already created</p>
                      <p className="text-white/40 text-sm">You can proceed to the next step</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-2 pt-3">
                ...
                <button
                  onClick={hasAssistant ? () => setCurrentStep(3) : handleCreateAssistant}
                  disabled={isLoading}
          ...
          {/* Step 3: Calendar (moved before phone) */}
          {currentStep === 3 && (
            <div className="space-y-5">
              <div>
          ...
          {currentStep === 4 && (
            <div className="space-y-5">
                ...
                <div className="bg-[#111114] border border-[#303030] rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-white/40 text-sm">You can proceed to complete setup</p>
                    </div>
                  </div>
                </div>
              ) : (
              ...
              <div className="flex gap-2 pt-3">
                ...
                <button
                  onClick={hasPhoneNumber ? () => setCurrentStep(5) : handleSavePhone}
                  disabled={isLoading || (!hasPhoneNumber && phoneTab === 'new' && (!areaCode || areaCode.length !== 3))}
                  className="flex-1 bg-white text-black text-sm font-medium py-1.5 px-3 rounded-lg hover:bg-white/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {isPurchasingNumber ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                  {hasPhoneNumber ? 'Continue' : 'Next'}
                  <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-[#111114] border border-[#303030] rounded-lg p-4 space-y-2">
                <div className="flex items-center gap-2 text-sm">
                  {hasBusiness ? <Check className="w-4 h-4 text-green-500" /> : <div className="w-4 h-4 rounded-full border border-white/20" />}
                  <span className="text-white/60">Business:</span>
                  <span className="text-white ml-auto">{hasBusiness ? 'Created' : 'Skipped'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  {hasAssistant ? <Check className="w-4 h-4 text-green-500" /> : <div className="w-4 h-4 rounded-full border border-white/20" />}
                  <span className="text-white/60">Assistant:</span>
                  <span className="text-white ml-auto">{hasAssistant ? 'Created' : 'Skipped'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  {hasCalendarConnected ? <Check className="w-4 h-4 text-green-500" /> : <div className="w-4 h-4 rounded-full border border-white/20" />}
                  <span className="text-white/60">Calendar:</span>
                  <span className="text-white ml-auto">{hasCalendarConnected ? 'Connected' : 'Skipped'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  {hasPhoneNumber ? <Check className="w-4 h-4 text-green-500" /> : <div className="w-4 h-4 rounded-full border border-white/20" />}
                  <span className="text-white/60">Phone:</span>
                  <span className="text-white ml-auto">{hasPhoneNumber ? 'Assigned' : 'Skipped'}</span>
                </div>
              </div>
